...
const FirebaseDB = (() => {
  ...
  return {
    ...
    // Cria uma conta nova no Firebase Authentication SEM deslogar quem está
    // usando o app agora. Truque: usa um "app" Firebase secundário só para essa
    // criação (o SDK do Firebase loga automaticamente como o usuário recém-criado
    // no app em que a criação foi feita — se fizéssemos isso no app principal,
    // o admin seria deslogado e logado como o novo funcionário).
    async criarUsuarioAuth(email, senha) {
      const NOME_APP_SECUNDARIO = 'appSecundarioCriacaoUsuario';
      let appSecundario = firebase.apps.find(a => a.name === NOME_APP_SECUNDARIO);
      if (!appSecundario) appSecundario = firebase.initializeApp(firebaseConfig, NOME_APP_SECUNDARIO);
      ...
      try {
        ...
        await authSecundario.signOut().catch(() => {});
        throw erro;
      }
    },
    // Envia e-mail de redefinição de senha (única forma client-side de trocar a
    // senha de OUTRO usuário — o Firebase não permite isso diretamente sem o
    // Admin SDK/backend). O próprio usuário define a nova senha pelo link recebido.
