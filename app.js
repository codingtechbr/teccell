...
/* ===========================================================
   AUTENTICAÇÃO (usuários locais — ver README para migrar a
   Firebase Authentication real)
=========================================================== */
async function garantirUsuariosSeed() {
...
$('#btn-criar-admin').addEventListener('click', async () => {
  ...
  if (usuarios.some(u => u.usuario === 'admin')) {
    toast('Usuário admin já existe. Faça login normalmente.', 'aviso');
    return;
  }
  await DB.add('usuarios', { usuario: 'admin', senha: 'admin123', nome: 'Administrador', nivel: 'Admin', ativo: true });
  toast('Usuário admin criado! Login: admin / Senha: admin123');
});
...
$('#form-login').addEventListener('submit', async (e) => {
  ...
  if (encontrado.ativo === false) {
    erroEl.textContent = 'Este usuário está inativo.';
    return;
  }
  state.usuario = encontrado;
  sessionStorage.setItem('techstore_sessao', JSON.stringify(encontrado));
  await iniciarApp();
});

function logout() {
  state.usuario = null;
  sessionStorage.removeItem('techstore_sessao');
...
(async function boot() {
  const sessao = sessionStorage.getItem('techstore_sessao');
  if (sessao) {
    state.usuario = JSON.parse(sessao);
    await iniciarApp();
  }
})();
...
function formularioUsuario(u = {}) {
  return `
    <form id="form-usuario" class="form-grid">
      ...
      <label>Usuário (login)*<input type="text" id="u-login" value="${u.usuario||''}" required ${u.id?'readonly':''}></label>
...
function abrirModalUsuario(usuarioExistente = null) {
  ...
  $('#form-usuario').addEventListener('submit', async (e) => {
    e.preventDefault();
    const dados = { nome: $('#u-nome').value.trim(), usuario: $('#u-login').value.trim(), senha: $('#u-senha').value, nivel: $('#u-nivel').value, ativo: $('#u-ativo').value === 'true' };
    if (usuarioExistente) { await DB.update('usuarios', usuarioExistente.id, dados); toast('Usuário atualizado!'); }
    else {
      const usuarios = await DB.getAll('usuarios');
      if (usuarios.some(u => u.usuario === dados.usuario)) return toast('Já existe um usuário com esse login.', 'erro');
      await DB.add('usuarios', dados); toast('Usuário criado!');
    }
